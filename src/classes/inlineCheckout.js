...
export class InlineCheckout extends BaseInlineCheckout {
  ...
  #updatePayButton(data) {
    try {
      ...
      const btnTextContent =
        data?.textContent || `<div class="pay-button-text">Pagar $${this.cartTotal}</div>`;
  ...
  async _checkout() {
    ...
    let cardTokens;
    const selected_apm = this.apmsData
      ? this.apmsData.find(iapm => iapm.pk === this.radioChecked)
      : {};
    ...
    try {
      const customerData = await this._getCustomer(this.customer, this.abortController.signal);
  ...
  async #handleSaveCard(auth_token, businessId, cardTokens) {
    ...
    if (
      ...
      try {
        await saveCustomerCard(this.baseUrl, auth_token, this.secureToken, businessId, {
  ...
  #handleOpenCloseAccordion(type = "", position = null, closeAll = false, closeOthers = false) {
    ...
    accordions.forEach(({ accordion, type: currentType }) => {
      ...
      if (position !== null && currentType === type && accordion.open) {
        accordion.open(
          currentType !== "paymentMethods" ? position : position - (this.#cantCustomerCards + 1),
        );
      }
    });
  }
